...
//	@Scheduled(cron="0 0 * * * *") // 정시 마다(0분 0초 마다) (실제 운영 시)
	public void imageDelete() {
		...
		File profileForder = new File(profileLocation);
		File boardForder = new File(boardLocation);
		
		// 2. 서버에 저장된 파일 목록 조회
		File[] profileArr = profileForder.listFiles();
		File[] boardArr = boardForder.listFiles();
