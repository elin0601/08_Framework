...
public class EmailServiceImpl implements EmailService{
	...
	public String sendEmail(String htmlName, String email) {
		...
		try {
			...
			switch(htmlName) {
			case "signup" : subject = "[boardPreoject] 회원 가입 인증 번호 입니다.";
			break;
			...
			// MimeMessageHelper : Spring에서 제공하는 메일 발생 도우미(간단 + 타임리프)
		...
		// 1) 해당 이메일이 DB에 있을 수 있기 때문에
		//    수정(update)을 먼저 진행
		//    -> 1 반환 == 업데이트 성공 == 이미 존재해서 인증 번호를 변경
 ...
 *  --> 이메일 메시지를 보내고 받을 때 사용하는약속(규약, 방법)
 *  
 * - Google SMTP
 * 
