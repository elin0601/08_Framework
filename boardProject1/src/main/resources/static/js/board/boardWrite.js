...
/**
 * @param inputImage : 파일 선택된 input 태그
 * @param order : 이미지 순서
 */
const changeImageFn = (inputImage, order) =>{
    ...
    // ------ 파일 선택한 다음 취소 해서 파일이 없는 경우 ------
    if(file == undefined){
        console.log("파일 선택 취소 됨");
        ...
        inputImage = temp; // 원본 변수의 백업본을 참조할 수 있게 대입
    ...
    if(file.size > maxSize) {
        ...
        // 요소는 있는데 값이 없는 경우 == 아무 파일도 선택된 적이 없을 때
    ...
    reader.addEventListener("load", e => {
        ...
        // img 태그(.preview)에 src 속성으로 url 값 대입
