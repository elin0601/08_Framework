...
if( updateInfo != null ) {
    ...
    updateInfo.addEventListener("submit", e => {
        ...
        const addr1 = memberAddress[1].value.trim().length === 0; // true/false
        const addr2 = memberAddress[2].value.trim().length === 0; // true/false

        // 모두 true인 경우만 true 저장
        const result1 = addr0 && addr1 && addr2; // 아무것도 입력 X

        // 모두 false일 때만 true 저장
...
if (changePw != null) {
    ...
    changePw.addEventListener("submit", e => {
        ...
        else if (newPwConfirm.value.trim().length == 0) str = "세 비밀번호 확인을 입력해 주세요.";
