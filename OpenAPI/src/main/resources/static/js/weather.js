...
// regionValue : 지역명(서울, 부산 ...)
const getUltraSrtFcst = (regionValue) => {

    // 데이처를 요청해서 응답 받을 주소(CallBack URL)
    const curl = "http://apis.data.go.kr/1360000/VilageFcstInfoService_2.0/getUltraSrtFcst";

    // 예보 지점 X/Y 좌표
    // const nx = 60;
    // const ny = 127;
    const nx = coordinateList[regionValue].nx; // x 좌표
    const ny = coordinateList[regionValue].ny; // y 좌표
    ...
    .then(result => {
        ...
        // 현재 시간으로 부터 가장 가까운 예보를 저장할 저장할 빈 객체 생성
        const weatherObj = {};
...
region.addEventListener("change", e => {
    ...
    getUltraSrtFcst(e.target.value); // 해당 지역 날씨 조회 실행
