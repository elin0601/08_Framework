...
const updateCancle = document.querySelector("#updateCancle"); // 취소
...
function getTotalCount(){ // 함수 정의
    ...
    // 비동기로 서버(DB)에서 전에 Todo 개수를 조회하는
    // fetch() API 코드 작성
    // (fetch : 가지고 오다)
...
function getCompleteCount(){
    ...
    .then(result => {
        ...
        // #completeCount 요소애 내용으로 result 값 출력
...
addBtn.addEventListener("click", ()=> {
    ...
    .then(resp => resp.text()) // 반환 된 값 text로 변환

    // 첫 번째 then에서 반환된 값 중 변호나된 text를 temp에 저장
    .then(temp =>{
        if(temp>0) { // 성공
            alert("추가 성공");
...
const selectTodoList = () => {
    ...
    .then(result => { // result == 첫 번째 then에서 반환된 결과값
        ...
        console.log(todoList); // 객체 배열 형채 확인!
        ...
        for(let todo of todoList) { // 향상된 for문
            ...
            for(let key of arr) {
                ...
                if(key === 'todoTitle'){
                    ...
                    a.innerText = todo[key]; // 제목을 a태그 내용으로 삽입
...
// 수정 레이어에서 취소 버튼(#updateCancle이 클릭 되었을 때)
...
// js 파일에 함수 호출 코드를 작성 -> 페이지 로딩 시 바로 실행
getCompleteCount();
getTotalCount();
selectTodoList();
